#This code is used to compute the un-normalized spectra from the clusters corresponding to the scans used for the k-means.
#And creating plots to show the opacity window effect
...
prof_H = np.zeros((10,5,32)) # This is where the profiles will be stored. 
prof_Ca = np.zeros((10,5,41))# This is where the profiles of Ca will be stored. 

clusts = [48,12,49,25,40]
...
mask1 = np.zeros((1518,1641,4))

wav= readsav(dpath_kmean_bose+'IRIS_wavs.sav')
wav_mg= wav['wav_mg']
dopp_Mg = ((wav_mg[40:75]-2796.35)/2796.35)*3e5

clust_Mg_49 = readsav(dpath_kmean_bose+'cluster_49_Mg.sav')
clust_Mg_49 = clust_Mg_49['mg_clust49']
#clust_Mg_49= clust_Mg_49/np.mean(clust_Mg_49[150:155])

clust_Mg_25 = readsav(dpath_kmean_bose+'cluster_25_Mg.sav')
clust_Mg_25 = clust_Mg_25['mg_clust25']
#clust_Mg_25= clust_Mg_25/np.mean(clust_Mg_25[150:155])

clust_Mg_12 = readsav(dpath_kmean_bose+'cluster_12_Mg.sav')
clust_Mg_12 = clust_Mg_12['mg_clust12']
#clust_Mg_12= clust_Mg_12/np.mean(clust_Mg_12[150:155])
clust_Mg_avg = readsav(dpath_kmean_bose+'cluster_avg_Mg.sav')
clust_Mg_avg = clust_Mg_avg['mg_clust_avg']

cmap1= colors.ListedColormap(['magenta','lime','blue','red'])
...
axs[0].imshow(np.transpose(cubeH[:,:,scan[3],8]),cmap='Greys_r',extent=[0,56,0,61],origin='lower',aspect='auto')
axs[0].contour(np.transpose(mask1[:,:,0]),levels=1,colors=['magenta'],alpha=0.8,extent=[0,56,0,61],origin='lower',aspect='auto')
...
axs[1].contour(np.transpose(mask1[:,:,0]),levels=1,colors=['magenta'],alpha=0.8,extent=[0,56,0,61],origin='lower',aspect='auto')
...
axs[1].set_title(r'CHROMIS Ca II K -24.56 km s$^{-1}$')
axs[1].set_ylabel('arcsec')

axs[2].imshow(np.transpose(cube_Mg[:,:,scan[3]]),cmap='Greys_r',extent=[0,56,0,61],origin='lower',aspect='auto')
axs[2].contour(np.transpose(mask1[:,:,0]),levels=1,colors=['magenta'],alpha=0.8,extent=[0,56,0,61],origin='lower')
...
axs[3].plot(dopp_H, result_prof_H[4,:]/1e3,color='black',linestyle='dashed')
axs[3].set_xlabel(r'$\Delta$ $\lambda$[km s$^{-1}$]')
axs[3].set_ylabel('I$_{counts}$[x1000]')
axs[3].set_title(r'H $\alpha$')
axs[3].plot(dopp_H,np.abs(result_prof_H[4,:]/1e3-result_prof_H[0,:]/1e3)/1.3, linestyle='dotted',color='magenta')
#axs[3].plot(dopp_H,np.abs(result_prof_H[4,:]/1e3-result_prof_H[1,:]/1e3)/2, linestyle='dotted',color='lime')
#axs[3].plot(dopp_H,np.abs(result_prof_H[4,:]/1e3-result_prof_H[2,:]/1e3)/2, linestyle='dotted',color='blue')
#xs[4].axvline(x=-38,color='gray',linestyle='-.',zorder=-1)axs[3].plot(dopp_H,np.abs(result_prof_H[4,:]/1e3-result_prof_H[3,:]/1e3)/2, linestyle='dotted',color='red')
axs[3].axvline(x=-38,color='gray',linestyle='-.',zorder=-1)
...
axs[4].plot(dopp_Ca, result_prof_Ca[4,:41],color='black',linestyle='dashed')
...
axs[4].plot(dopp_H,np.abs(result_prof_H[4,:]/1e3-result_prof_H[0,:]/1e3)/2.5e9, linestyle='dotted',color='magenta')
#axs[4].plot(dopp_H,np.abs(result_prof_H[4,:]/1e3-result_prof_H[1,:]/1e3)/2e9, linestyle='dotted',color='lime')
#axs[4].plot(dopp_H,np.abs(result_prof_H[4,:]/1e3-result_prof_H[2,:]/1e3)/2e9, linestyle='dotted',color='blue')
#axs[4].plot(dopp_H,np.abs(result_prof_H[4,:]/1e3-result_prof_H[3,:]/1e3)/2e9, linestyle='dotted',color='red')
axs[4].axvline(x=-38,color='gray',linestyle='-.',zorder=-1)

axs[5].plot(dopp_Mg,clust_Mg_12[40:75]/1e2,color='lime')
axs[5].plot(dopp_Mg,clust_Mg_49[40:75]/1e2,color='blue')
axs[5].plot(dopp_Mg,clust_Mg_25[40:75]/1e2,color='red')
...
cbar.set_ticks(tick_locs/3.)
cbar.set_ticklabels([3,2,1,0])

cbar.ax.set_title("RP Index No.")
#fig, ax = plt.subplots(figsize=(1,6))
#lab
#plt.colorbar(cmap=cmap1,label=[48,12,49,12],ticks=range(4))
plt.show()
#plt.tight_layout(fig,rect=[0,0,0.95,1],h_pad=0.5)
